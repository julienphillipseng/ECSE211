...
public class Odometer extends OdometerData implements Runnable {
	...
	private static final double RAD_TO_DEG = 57.2598;
	...
	public void run() {
		...
		while (true) {
			...
			double distL = PI * WHEEL_RAD * (leftMotorTachoCount - lastTachoL) / 180; // compute wheel
			double distR = PI * WHEEL_RAD * (rightMotorTachoCount - lastTachoR) / 180; // displacements
			...
			Theta = position[2] / RAD_TO_DEG; // Converting Theta back to radians for the calculations
			...
			odo.update(dX, dY, deltaT * RAD_TO_DEG);
